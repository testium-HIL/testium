...
`src/testium/interpreter/test_report/test_report.py` — `_EXPORTER_REGISTRY` dict maps a format name (key in the YAML `report.export`) to a lazy loader. Built-ins: `text`, `json`, `junit` (needs `junit_xml`), `html` (needs `lxml`). They run **in-process** on the live connection; their deps are hard requirements bundled in every channel. `sqlite` is the storage layer, no-op as an export.

Third-party plugins (`entry_points(group="testium.exporters")`) are discovered and executed on the **host** Python (`bins.python_bin()`, same philosophy as `py_func`/`pytest`): the bundled interpreter of Flatpak / AppImage / PyInstaller is read-only, so an in-process scan could never see a user `pip install`. Any non-builtin format routes to `src/testium/runtime/export_worker.py` — stdlib-only, launched as `python3 export_worker.py '<json>'` (format, db, path, pats, keys, no_header, name). It lives in `runtime/` so it is bundled at the `_MEIPASS` root by the PyInstaller spec and staged under `/tmp` for Flatpak by `bins._get_host_testium_path()`. The report DB is first materialized to a temp SQLite file with the `sqlite3` backup API (mandatory: the live DB may be `:memory:`, and the Flatpak host only sees shared paths). Worker exit 3 + `__TESTIUM_EXPORT_FORMATS__=` sentinel → unknown format; the skip message combines the static builtin list with the host plugin list.

```toml
[project.entry-points."testium.exporters"]
my_format = "my_pkg:MyExporter"
```
Exporter contract (unchanged): `__init__(self, name, con, path, pats, keys, no_header=False)` — the class does its work in `__init__` and writes to `path`. The worker opens its own `sqlite3` connection on the temp copy.

**`testium_report` helper** (`src/testium/runtime/testium_report.py`, stdlib-only): `Report` (header dict, `rows(pats, keys)` with parameterized LIKE-OR filters, `tree()` via parent_id), `TestRow` (typed row; `passed/failed/skipped`, `duration_s` = ticks/10000, `data` JSON-decoded, `to_dict()` reproducing the historical JSON-export keys/order), `Exporter` (implements the contract, drives `export()`), `prepare_file_to_save` (moved here; `interpreter/utils/paths.py` delegates). Three import names for the same file: `testium_report` in the export worker (worker launched by path → `runtime/` is sys.path[0], so plugins import it with zero install and the version always matches; also from a wheel install via the top-level shim `src/testium_report.py` + `py-modules` — the runtime copy precedes site-packages at execution), and `runtime.testium_report` in-process. The four built-in exporters subclass `Exporter` (old `report_export.py` base deleted); rendering byte-identical (verified by diffing all formats produced by both code versions on the same database). Constraint: the helper must stay free of testium imports.

The `command` export runs an arbitrary external tool the same way: `- command: {cmd: "mytool $(db) $(out)", path: ..., file_name: ...}` — `$(db)`/`$(out)` follow the standard `$(...)` syntax but are masked from `expanse()` with NUL sentinels (a global named `db`/`out` must not capture them) and substituted per token after `shlex.split` (paths with spaces stay one argument); spawned on the host (`flatpak_host_spawn` in Flatpak, AppImage env scrubbed via `apply_host_libs`).

Behaviour on errors (run always continues):
- Unknown format → info line `[report] Export skipped: format "X" not found. Available: ...`.
- Builtin optional dependency missing → same info line with a pip-install hint.
- Worker / command failure or non-zero exit → `[report] Export skipped: ...` with the stderr detail.

A real-world test plugin lives at `test/validation/fake_exporter/` (CSV exporter, installed by `test/validation/run.sh` into the validation venv used as `python_bin`, exercised by `test/validation/items/report_plugin/` together with the `command` export).
...
**Windows (`build_all.ps1`).** The Windows-native counterpart builds the channels that apply on Windows into `dist/`: the wheel, the one-folder PyInstaller exe (zipped as `testium-<v>-win64.zip`), the Inno Setup installer (`testium-<v>-setup.exe`) and the manual PDF (needs a LaTeX toolchain via Git Bash; skipped with a warning when missing). Flatpak/AppImage have no Windows equivalent and are absent. It reuses the `run.ps1` venv at `test/tmp/testium_venv` and skips a step whose artifact already exists. Options are aligned on `build_all.sh`: `--clean` forces a rebuild; `--serial` changes nothing (the Windows steps are dependent, the build is always serial); `--ram` is ignored (no tmpfs).
...
The schema is the realized source of truth for the LSP server (`testium lsp`), the `testium schema` CLI dump, and future auto-generated manual sections: `lsp/schema.py` reads both `PARAMS` and `ACTIONS` **purely from class attributes** — no `inspect.getsource`/AST parsing. This is what lets the full schema (including nested actions) survive a frozen PyInstaller build where the `.py` source isn't on disk.

`testium schema` emits a **JSON Schema (draft 2020-12)** describing valid `.tum` files: each item has its own `$def`, action parents (`console`, `plot`, `json_rpc`) carry a strictly-constrained `steps` array, common parameters apply via property inheritance. `$id` is a versioned URN (`urn:testium:tum-schema:<version>`) — opaque, no fetch required. Built from each `Param`'s declarative shape (`kind`, optional `type`, `doc`, `required`, `default`). Single source of truth: the LSP server (`testium lsp`) consumes the same schema, external validators (yaml-language-server, ajv, check-jsonschema) accept it directly, AI agents read it as a formal contract.

`schema/tum.json` is the committed, public snapshot of the generated schema (regenerated with `testium schema > schema/tum.json` when `PARAMS`/`ACTIONS` change) so that any agent reading the repo, or any yaml-language-server config pointing at a local file or the raw GitHub URL, finds it without running testium — it is the only tracked generated artifact at the repo root, and deliberately so (see `schema/README.md`). The positive fixtures exercising every item type live with the validation suite in `test/validation/schema/`. `test/validation/schema_check.py` (called by `run.sh` before the suite) verifies that (1) the live `testium schema` output is a valid draft-2020-12 schema, (2) the versioned file is in sync, (3) every fixture validates — drift in any direction fails the suite.
...
- `console` `exec` action — command completion by injected marker (`api/console_sync.py`): the sent line is `<cmd>` plus a per-call marker print (`@TSTM:<uuid12>@`); completion = marker seen at line start. Echo defenses: the sent line never contains the marker in one piece (sh: adjacent-quote split; cmd: caret split; powershell: string concatenation) and the completion regex is line-anchored, so a raw or expanded echo of the command line cannot match. Each dialect emits a blank line before the marker (output without a trailing newline). The regex reserves an optional `:digits` field so a future exit-code emission changes nothing in detection (exit-code exposure was considered and declined). `dialect` is fixed at `open` (guessed for terminal/ssh from the shell, `none` for telnet/rawtcp/serial where `exec` refuses); a nested shell speaking another dialect is documented as unsupported — no per-exec override. A prompt-declaration mechanism (`prompt` at open + `wait_prompt`) was designed and dropped as superfluous: non-shell consoles keep `writeln` + `read_until`. Foundations reworked in the same effort: `read_until` reads chunks (`read_available`, per-transport overrides) with incremental UTF-8 decoding and a pushback buffer (`Console._pending`, or the native byte store for term/buffered-serial) preserving the stream-positioned-after-match invariant; regex scanned once per chunk over the 64 KiB tail; `time.monotonic` deadline replaces the `threading.Timer`; the timeout-0 drain loop polls `should_stop`; rawtcp/unbuffered-serial honor their read timeout; `writeln` line ending is an `open` param (`newline: lf|crlf|cr`).
- Alternative Textual (TUI) and NiceGUI (web) views were prototyped on the `gui/` presenters and removed (visual quality below the Qt reference). What stays for a future non-Qt view: the presenters, the `UI_BACKENDS` registry in `main()` as the extension point, and the shared item-state snapshot format (`tree_presenter`), all proven by the prototypes.
- Install model (0.6): `pip install testium-hil` ships everything — pyside6 and pygls are plain dependencies, the `[qt]`/`[lsp]`/`[all]` extras are removed (the 0.5 core/extra split traded install size for a complicated first contact; reversed). GUI dispatch keeps the lazy backend registry in `main()` (reinstall hint when the import fails); `require_qt()` still guards dialog/plot items for stripped environments.
- GUI overhaul 0.5 (branch `gui-overhaul`). Layout: four dockable panels (Log, Test doc, Test item, Variables — the last two replace the F1 window), closable/movable/tabbable, View menu with toggleViewActions + "Reset layout"; window state versioned (`STATE_VERSION`, older blobs reset once to the default layout: log right, doc/item/variables tabbed below it); corner ownership set AFTER `restoreState` (it restores corners too) so the bottom area sits under the tree, not across the window; layout saved on exit whatever the run state. Tree: per-row Result `QComboBox` replaced by plain single-line text + tooltip + dated click history (`gui/result_history.py`); duration column always built, visibility follows the preference live; `Columns` per tree instance (module-level mutable `TEST_COLS` retired); system fonts; follow-run button (scrolls to each starting step, respects folds, manual scroll disengages) and search button in the control row. Architecture: humble-view split — presenters in `src/testium/gui/` import no toolkit (AST-enforced by `gui_purity_check.py`): `run_presenter` (state machine, run log, timers via a `Scheduler` protocol, one `RunUiState` push replacing 20 setEnabled calls), `file_presenter` (owns the process triad and recents; weak-ref broadcast across windows), `tree_presenter` (per-file state store, LRU; path-keyed fold/check/breakpoint snapshot format and restore, re-issuing breakpoints with the new interpreter ids), `status_presenter` (engine status decoded to display directives: result icon incl. success-after-fail orange, failure count, one-line result + tooltip, highlight, follow target = topmost collapsed ancestor, first-timestamp latch, global verdict), `tree_view_model` (per-type visual config, column table, gutter/highlight precedence and palette, search match rule), `debug_presenter` (context-menu availability — attach py_func-only, jump paused/non-skipped/outside Parallel —, breakpoint-condition upgrade/downgrade, debug-output preference and effective-value sync), `preferences_presenter` (declarative field table, store/restore/dirty set, changed-preference -> effect dispatch), `session_presenter` (geometry/state blobs, layout migration, startup log/file decisions), `open_target` (open-a-path cascade shared by log clicks/item dock/editor jump, log keyword styles + clickable-path pattern + `@@ts@@` marker), `run_io` (status-queue dispatch, output line pump, log tee), `search_presenter`, `variables_presenter`, `result_history`; `main_win/` implements the view protocols (`gui/protocols.py`), delegating properties keep the pinned API (`win.runner.state`, `win.test_service`, `win.file_manager.reload`, ...); `qt_scheduler.py` maps the Scheduler onto QTimer. Deliberate deviations from the plan: no LogPresenter (two preference writes), no central status router (each status has one consumer). Item dialogs: shared `dialog_presenter.py` (auto-close decision, argv access, `[result, success]` protocol, JSON last-value store replacing the per-dialog QSettings — previous selections are not migrated —, cx_freeze stream mute) + `choices_presenter.py` (selection serialization/restore, select-all/fold tri-state transitions); shared fold/cascade in `interpreter/utils/tree_states.py` (importable by the spawned dialog children AND `gui/`); dialogs resizable. Presenter-level regression: `gui_presenter_check.py` runs the run/file/variables/tree/status/debug/preferences/session presenters, `open_target`, `run_io` and the dialog helpers against fakes with no PySide6 import. Log font: the size preference is the single size source; the size embedded in the stored font string is ignored.
- Live test-flow control (branch `runtime-flow-control`): `StepController` extended into the run-level controller — pause request (a level: Pause lets the current step finish and the *next* item pauses in its `run_test_init`, fixing the 116e528a regression where pause only marked already-running items, all past their spin), jump state and last-executed leaf. Jump (`jump_to`/`jump_back` commands, paused-only, refused into/out of `parallel` and onto unchecked/skipped targets): the paused item's spin picks the armed jump, does its bookkeeping (`_abandon_for_jump`: `decLevel` for containers, NORUN "interrupted by jump", `finished` status, no report row) and raises `JumpRequested`; every unwound ancestor rebalances in the `@test_run` wrapper's `except`; the root converts it (`take_jump`) and re-enters from child 0 in seek mode — non-ancestors of the target fall through instantly with a fresh NORUN `TestResult` (no status/log/report), ancestors re-execute normally (duplicate container report rows, as cycles already produce), the target consumes `arrived()` and pauses on itself. `cycle`'s bare `except:` and `parallel`'s branch catch re-raise `JumpRequested` explicitly. New `paused` status message replaces the GUI's `started`+`isBreakpoint` pause inference (wrong for conditional breakpoints); `TestRunner.on_paused` idempotent. Conditional breakpoints: `add_breakpoint(condition=)`, `<| ... |>` expanded at `run_test_init`; unusable expression → WARN + pause anyway; hollow-ring gutter icon, condition in the 5th field of the path-keyed item states (length-tolerant restore). Checkboxes live during runs: signals stay connected (`updateStatus`/`clearAllStatus` under `blockSignals`, the `paused` signal emitted after unblocking), no more `removeCheckBoxes` at run start (unchecked items no longer greyed mid-run; the box is the cue), `synchronizeEnabledState` batched into one `enabled_states` round-trip. `cont` clears `_is_paused` on **all** items (orphan re-freeze fix) and the spin clears it on stop (leak fix). "Re-run last step" (Shift+F10) = `jump_back`: the target re-executes (`arm_jump(pause_on_target=False)` → `arrived()` returns 'run' and arms an `into` step, so the *next* item pauses); the target is the last step recorded via `_is_step` (True on every item except group/cycle/parallel — NOT `is_container`, which unittest/pytest flip for report indentation), recorded at body *start* so a crashed step is still the target; a jump whose route crosses a cycle prints "Cycle "X" restarts at iteration 1". Root aggregation harmonized with group/cycle (only FAILURE fails the run): NORUN from seek-skips or an unchecked top-level item is neutral. Validation: `pause_jump_check.py` (5 scripted runs over the real queues), `gui_state_check.py` extended (condition persistence, legacy 4-field entries, signal-safe `updateStatus`).
- `restore_gd` restored the single load-time backup **by reference** (`update(dict)` after `clear_recursively`), so the next end-of-run clear emptied the backup's own nested objects in place: from the third run every dict-valued global was `{}` (a config-built `choices` menu reached `dialog_choices` as `[{}, {}]` → silently empty dialog, `[]` saved to QSettings, all-checked after reload; `console_instances`/`plot_instances`/py_func contexts were hollowed too). Fix: restore a `copy.deepcopy` of the backup (test_init.py). Guards added: `dialog_choices` FAILs with a clear message when the expanded list has no usable entry, and the dialog no longer overwrites the saved selection with an empty result. Validation: `gd_restore_check.py` (3 backup/restore cycles + non-aliasing assertion) and `dialog_choices` fixtures (absent before) with the menu coming from `param.yaml`.
- Debug UX consolidation (branch `debug_ux`): one user-facing Debug toggle on the run bar (`actionDebugOutput`, declared in the .ui like its siblings, Ctrl+Shift+D, bug icon ×3 themes) drives `test_debug` — persisted as the `debugOutput` preference, injected at load only when set (config files and `-d` keep priority, the action then mirrors the effective value via `get_gd_vars`), pushed live with `set_gd_var`, and preserved across the end-of-run `restore_gd` (`STICKY_GD = (test_debug, debug_rpc)`; GUI re-assertion after the `finished` status would race the restore). Per-item py_func IDE attach without editing the .tum: tree context menu → `set_debug_attach` control command (typed guard in `TestSet`) → runtime flag ORed with the `debug:` attribute; blue gutter dot (combined red+blue when a breakpoint is set); session-only by design — never in `getItemStates`, so a forgotten attach cannot block a later run. Developer/user split: `-g` renamed `--dev-debug` (`--debug` silent alias), `debug_rpc` + `--dev-debug` documented in CONTRIBUTING only; new manual chapter "Debugging your tests" unifies breakpoints, step-by-step, F1 variables, debug output and the py_func attach. Validation: `debug_output_check.py` (live toggle + sticky across runs), `py_func_attach_check.py` (pip-less venv as python_bin → deterministic fast failure proves the runtime flag reaches `execute()`).
- Error-message localization pass (branch `error_messages`, 308 messages audited, ~200 reworked). Mechanisms: `TestItemParams` carries its owning item (`owner=`) so `getParam`/`getParamAll` errors name the item, its `.tum` and the parameter — including the ~40 run-time `getParam` calls that were bare; the loader location prefix (`In: {path} > {item}`, test_set.py) now wraps every `ETUMError` raised during item construction, not only `ETUMSyntaxError`; an unresolved `$(name)` warns at its **consumption points** (not at first expansion, where being undefined is often normal): a parameter of a *running* item still carrying `$(...)` after expansion (`TestItemParams.expanse`, gated on `_is_running`, names item + variable), and a `<| ... |>` evaluation whose expression contains one (root cause reported instead of the SyntaxError; `evaluate()` used to swallow every exception outside debug mode — including a dead eval subprocess turning all evals into silent no-ops). `params.warn_once(key, message, debug_hint=None)`: per-load registry reset by `prepare_global`, optional remedy printed as a companion DEBUG line (debug mode only); excluded names: loop vars, `db`/`out`/`result` deferred placeholders, NUL sentinels; the "To be reported to testium support team" family distinguishes user-fixable causes (dead/never-started subprocess → engine, function, file, resolved interpreter) from real internal invariants ("Internal error, please report it."). Message format rule: origin (ETUM* `file=` arg or item context) + offending value (raw and expanded when expansion is involved, model `test_init.py` parameter-file message) + remedy when one exists. `[report] Export skipped:` lines always carry the format and output path. Correctness fixes found by the audit: sleep `self.name()`, `python_env` naming, report `key` vs `pattern`, jsonrpc XOR wording, plot missing f-string, `py_eval` KeyError destroying eval errors, lua `string.format` missing argument, `handle.py` broken `print_exception` call, raw-TCP silent failed open, dead ssh key check; lua_func worker bind/connect failures reach stderr and exit non-zero (was silent exit 0).
- `py_func` step debugging (`debug: true` item attribute): the worker calls `debugpy.listen(("localhost", port))` + `wait_for_client()` before `func_exec` (`py_func/handle.py:_debug_attach`), so the user attaches an IDE and lands on their own breakpoints. Port from the `py_func_debug_port` global (default 5678), carried in the RPC payload — not at spawn, because a persistent `context_id` process can be created by a non-debug item. listen-once module guard (debugpy forbids a second `listen`; the port is remembered only on success, a later different port is ignored with a warning). debugpy must be installed on the host interpreter (`python_bin`), like pytest; missing → plain skip-style failure message, plus a parent-side `print_warn` ("debug setup failed … the test continues") so the non-pausing run is explicit (message-content detection: `"debugpy" in ret`, the messages are ours). Verified: the daemonized debugpy adapter (`close_fds=True`) does not interact with the RPC stdout redirection or the drain; SIGTERM (Stop) kills a worker blocked in `wait_for_client()`; the 3600 s RPC call timeout bounds an unattended wait; ephemeral successive listens are fine (SO_REUSEADDR). Validation: busy-port `_FAIL` item (fails fast whether debugpy is installed or not).
- `json_rpc` UDP not-open socket guard (`jsonrpc_adapters.py`): the socket is shared through the global dict keyed by `rcv_port`; a `query`/`receive` without a prior `open` action (or with a different `rcv_port`) fetched `None` and crashed with `AttributeError: 'NoneType' object has no attribute 'sendto'` (raw traceback). `_opened_sock()` raises a plain `ETUMRuntimeError` naming the port and the fix (run `open` first / align `rcv_port`), reported as a clean one-line FAILURE by the actions' existing `except ETUMRuntimeError` branch. Validation: `UDP query without open action` `_FAIL` item (no server needed). Related display fix (`item_actions/action.py`): an action without a `name:` has no banner/footer, so its failure message was never printed to the log (only the bare result) — `write_footer` now prints the message on FAILURE even without a banner.
- `py_func` error reporting (`py_func/handle.py`, `py_func/func_call.py`): the `except TypeError` around `func_exec` labelled **any** TypeError raised inside user code as `calling function with bad arguments` — wrong diagnosis, no traceback, no line number. Replaced by an explicit pre-call signature check (`inspect.signature().bind()`, applied to plain functions and `FunctionItem.exec`) that reports the expected signature vs the given params; every exception raised by user code now yields a full traceback trimmed of testium-internal frames (`format_user_traceback()`: drops `handle.py`/`func_call.py`/`<frozen importlib>` leading frames so it starts in the user's file — also applied to module import errors). Unknown `func_name` gets an explicit message listing the module's callables instead of an AttributeError. `ETUMError.__str__` wraps per physical line, preserving embedded newlines (a wrapped traceback used to collapse into one re-flowed blob). Lua side (`lua_func/handle.lua`): `pcall` → `xpcall` + `debug.traceback` so failures report the user's call stack, not only the innermost `file:line`; the trace is cut at the `xpcall` frame (testium internals hidden), and the call goes through a closure because Lua 5.1's `xpcall` has no vararg forwarding (closure `return f(...)` still propagates multi-value returns).
- `testium_report` exporter helper: stdlib-only module in `runtime/` giving plugins and built-ins one API over the report DB (typed rows, parameterized pattern/key filters, tree, output-file rotation) — no SQL or schema knowledge needed, importable by plugins with zero install (worker sys.path) and from the wheel for dev (`py-modules` shim). Built-ins ported onto it, `report_export.py` deleted, byte-identical outputs verified. `pyproject.toml`: `packages.find` excludes stale `src/build/` trees from the wheel. See "Report exporters & plugins".
- Run/debug bar: `actionStart_test` (Start/Pause toggle) and `actionStop_test` moved from the main toolbar (`.ui`) into the step bar (`_build_step_bar`), now titled "Run" and **always visible**. IDE-convention shortcuts added in the `.ui`: F5 (start/pause/resume — one toggling action) and Shift+F5 (stop), completing F10/F11/Shift+F11 (objectName stays `stepBar` so saved window states keep its position; `setVisible(True)` forced after `restoreState` because older settings saved it hidden). `TestRunner.set_step_buttons_visible()` removed — enable/disable of the step actions is the only run-boundary change left. Adaptive look (`_update_step_bar_style`, refreshed on `topLevelChanged` — fires on undock and on re-dock after a drag — and after `restoreState`): main-toolbar style (24px + text) when docked in the top area, compact 16px icon-only on the other areas or floating.
- Stop while paused left the paused item highlighted green forever: its end-of-run status never arrives, so `updateStatus()` never resets it. `QTestTree.clearHighlights()` (signal-blocked walk) is called in `TestRunner.on_run_finished()`; a normal run end is unaffected (each item already resets on its final status).
- Report exporter rework: third-party `testium.exporters` entry-point plugins are no longer scanned in-process (impossible for users to install into the read-only bundled interpreter of Flatpak/AppImage/PyInstaller) but discovered and executed on the **host** Python via the new stdlib-only `runtime/export_worker.py`, on a temp SQLite copy of the report (`sqlite3` backup API — also covers the `:memory:` case). Built-ins stay in-process. New `command` export type runs any external tool with `$(db)`/`$(out)` placeholders. `fake_exporter` moved from the build venv (`scripts/build_env.sh`) to the validation venv (`test/validation/run.sh`), so `report_plugin` passes in every `--mode`. See "Report exporters & plugins".
- `json_rpc` UDP: `JrpcAdapter.receive()` skips request frames (a dict with `method` and neither `result` nor `error`) while waiting for a response, bounded by the timeout deadline. Needed because the multicast group membership (added with `IP_ADD_MEMBERSHIP`) makes the socket receive its **own** query looped back when `rcv_port == snd_port` — `check_answer` then rejected it as a malformed 1.0 answer. Also: the four json_rpc actions gained an `except ETUMRuntimeError` branch (plain one-line FAILURE, no traceback — the generic `except Exception` still dumps a traceback for unexpected errors), the `close` action no longer swallows its failure (`TestResult` was built but never assigned), and `ETUMError.__str__` wraps message lines longer than 100 characters. Validation: echo server `[jsonrpc_udp_request_echo]` mode resends the raw request before the response; a `_PASS` item covers the skip.
- Clean failure for configuration errors raised during item preparation: the `@test_run` wrapper now catches `ETUMRuntimeError` from `run_before_test()` and turns it into a one-line FAILURE of that item (message via `write_footer`), where it previously escaped to the top-level "TUM runtime error" banner with a traceback and could abort the campaign. First user: the `json_rpc` item validates its `udp:` block at run time (`server` non-empty string — with a dedicated hint for the `server: { var }` single-braces YAML-mapping trap, `snd_port`/`rcv_port`/`bufsize` numeric, `multicast_if` non-empty string, missing keys named individually) through `_config_error()`/`_describe()` helpers producing plain-language messages. Also benefits the pre-existing `ETUMRuntimeError` raises reached from `run_before_test` (e.g. console-adapter "console not found"). Validation: three `jsonrpc` `_FAIL` items (mapping server, missing key, textual port) run without the echo server.
- Path-keyed persistence of tree item states (GUI): fold, checked box and breakpoint survive a reload of the same file **and** a restart, tolerant to item additions/removals — replacing the positional fold/check restore guarded by an exact item-count match (which also leaked states to a different file of identical count). Key = root→item list of `(type, name, occurrence)` components computed GUI-side (`QTestTree._walk_with_keys`); occurrence counts only preceding siblings with the same `(type, name)`, so unrelated insertions don't shift keys; renamed/removed items silently lose their state. `getItemStates()`/`restoreItemStates()` (no count guard) re-issue `add_breakpoint`/`set_enabled_state` with the fresh interpreter ids (interpreter untouched — its ids are `id(self)` of the child process, regenerated each reload). Reload path (`TestFileManager.reload`) restores only when the reloaded path equals the previous one; settings persistence uses `itemStateList` + `itemStateFile` (old `checkList`/`foldList` entries no longer read). The positional `get/restoreCheckList`/`get/restoreFoldList` remain for the run-time checkbox add/remove cycle (same live tree). Also fixed `TestiumSettings.value()`: a missing `bytearray` key fed the bytearray default to `json.loads`, and the resulting exception `clear()`ed the whole in-memory config on first run (any setting written before the first geometry save was wiped). Headless regression: `test/validation/gui_state_check.py` (run.sh, source mode) — reload, reload with an added item (count changed), cross-file discard, restart.
- `json_rpc` UDP multicast (`jsonrpc_adapters.py`): sending to a 224.0.0.0/4 destination already set `IP_MULTICAST_TTL`, but the socket never joined the group, so only unicast replies were receivable — group-addressed replies (discovery-style traffic) were silently lost, and there was no way to pick the egress interface (multicast tests on a multi-homed host were nondeterministic). `_open()` now, for multicast destinations: sets `SO_REUSEADDR` (several members may share the port), `IP_MULTICAST_IF` when the new optional `udp: {multicast_if: <local-ip>}` is given, and joins the group with `IP_ADD_MEMBERSHIP` after bind. `is_ip_multicast()` no longer raises on malformed dotted-quads. Validation: the echo server (`items/jsonrpc/jrpc_echo_server.py`) gains a `[jsonrpc_multicast]` section with two group listeners on loopback — one replying unicast, one replying to the group — and `test.tum` exercises both flows plus a FAIL case (Linux-only, pinned to `multicast_if: 127.0.0.1` so host routing doesn't matter). Caveat noted in the test doc: on a single host the server's own membership already subscribes the interface, so the strict cross-host membership requirement can't be isolated there.
- Step-by-step execution (GUI): three actions — Step over (F10, next item at the same level), Step into (F11, next item at any depth), Step out (Shift+F11, run to the end of the current level, pause one level up). Interpreter side: a lock-protected `StepController` (`interpreter/utils/step_ctrl.py`) shared by every `TestItem` (assigned post-load by `TestSet`, like `report`); the item spinning in the `@test_run` pause loop registers as "current"; `step_*` commands (`process.py` command map → `TestSet.step_*`) arm mode + reference depth (`depth(item)` = number of ancestors, root = 0) and release only that item; the next item whose depth matches consumes the step in `run_test_init` and pauses itself — exactly the existing breakpoint pause point, so granularity is "before each item, at every depth". Rules: breakpoint hit disarms a pending step (breakpoint wins); `cont` resets; `stop`/`pause` disarm (so `execute_on_stop` teardown can't be re-paused); end-of-run resets in `TestSet.execute()`'s `finally`; skipped/disabled items don't consume a step (they return before `run_test_init`); `step_out` at depth 1 = run to the end; `run`/`unittest`/`pytest` have no internal pause points (into behaves as over). GUI side: actions defined in `testium_core_win.ui` (Test menu only — not in the main toolbar); a compact icon-only step bar (`MainWindow._build_step_bar`, 16×16, inserted above the tree like the search bar) is shown/hidden at run boundaries via `TestRunner.set_step_buttons_visible()`; menu entries and shortcuts stay, so F11 at idle starts the run paused on its first item (arm before `execute` on the single command thread). Breakpoint gutter: the `pause` column is moved to the far-left **visual** position (logical indexes untouched; `setTreePosition` pins the tree structure to the name column) with a fixed width computed from the style (icon size + the item delegate's `PM_FocusFrameHMargin + 1` decoration offset), and the marker is a red dot drawn in code (`_breakpoint_icon()`, `test_tree_item.py`) — no themed PNG. Enabled only while PAUSED (breakpoint or user pause); the GUI state stays PAUSED across a step, the tree highlight advances via the normal `started` statuses. Icons `step-{over,into,out}.png` × 3 themes. Headless regression: `test/validation/step_check.py` (run.sh, source mode) scripts the whole sequence through the real control/status queues.
...
- Parameters are expanded at **run time**, never at load: control-flow flags (`stop_on_failure`/`execute_on_stop`) resolve via properties at run, `cycle` iterator / `git` repo / `tested_references` references and `console` `telnet_port` are no longer (incorrectly) expanded or left unexpanded at load. Justified load-time exceptions: `name`, `doc`, `skipped`, and `unittest`/`pytest` `test_method`. Config-file values and include arguments follow the same rule: stored as declared, resolved at use through `globdict.resolve_gd` (read-side expansion memoized per generation, cycle guard keeps the declared text; `let` stays snapshot-on-write). A key set to `None` counts as defined. Remaining load-time spots: include/config file paths (the file must exist at parse) and jinja `{{ }}`/`{% %}` on globals (rendered with current values at load; include arguments are passed verbatim — `expand()`/`gd()` template helpers give explicit load-time resolution for structural use).
