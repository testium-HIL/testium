...
- Web GUI (`web_win/`, NiceGUI, `[web]` extra, `testium --ui web`): third client of the `gui/` presenters. Presenters mutate shared state; each browser gets a per-client page synced by timer (NiceGUI 3 has no shared client), so several viewers follow the same run. Tree with tick/expand models mapped to the engine, log pane, live variables table, run/step/breakpoint buttons; shares the item-state snapshot format. v1 limits as the TUI, plus plain-text log (no colours).
- Terminal GUI (`tui_win/`, Textual, `[tui]` extra, `testium --ui tui`): second client of the `gui/` presenters — tree with check/breakpoint/status glyphs (tables from `tree_view_model`/`status_presenter`), log pane styled from `open_target` rules, live variables table, key bindings for run/pause/step. Shares the item-state snapshot format with the Qt view. v1 limits: no preferences dialog, item dialogs still spawn the Qt children (or fail with the `[qt]` hint), no plots.
- Per-GUI install model: `pip install testium-hil` is the toolkit-free core (batch, `schema`, `lsp`); the Qt GUI is the `[qt]` extra (`[all]` = qt+tui+web+lsp). GUI dispatch via a lazy backend registry in `main()` (`--ui`, first installed backend by default, install hint when none); dialog and plot items fail with the `[qt]` hint when PySide6 is absent (`require_qt()`); wheel-mode validation installs `[qt,lsp]`.
