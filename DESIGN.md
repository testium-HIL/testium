...
- Per-GUI install model: `pip install testium-hil` is the toolkit-free core (batch, `schema`, `lsp`); the Qt GUI is the `[qt]` extra (`[all]` = qt+lsp). GUI dispatch via a lazy backend registry in `main()` (install hint when none); dialog and plot items fail with the `[qt]` hint when PySide6 is absent (`require_qt()`); wheel-mode validation installs `[qt,lsp]`.
