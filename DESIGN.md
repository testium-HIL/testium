...
**Windows (`build_all.ps1`).** The Windows-native counterpart builds the channels that apply on Windows into `dist/`: the wheel, the one-folder PyInstaller exe (zipped as `testium-<v>-win64.zip`) and the Inno Setup installer (`testium-<v>-setup.exe`); `-Manual` adds the PDF (needs a LaTeX toolchain via Git Bash). Flatpak/AppImage have no Windows equivalent and are absent. It reuses the `run.ps1` venv at `test/tmp/testium_venv`, skips a step whose artifact already exists, and `-Clean` forces a rebuild.
