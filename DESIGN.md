...
The `command` export runs an arbitrary external tool the same way: `- command: {cmd: "mytool {db} {out}", path: ..., file_name: ...}` — `shlex.split` + per-token `.format(db=…, out=…)` (same pattern as the `editor_cmd` preference), spawned on the host (`flatpak_host_spawn` in Flatpak, AppImage env scrubbed via `apply_host_libs`).
