...
class SessionView(Protocol):
    """Window-session surface driven by the session presenter. Blobs are
    opaque toolkit serializations."""

    def restore_geometry(self, blob) -> None: ...

    def restore_state(self, blob, version: int) -> bool: ...

    def apply_default_layout(self) -> None: ...

    def hide_doc_pane(self) -> None: ...

    def hide_log_pane(self) -> None: ...

    def save_geometry(self): ...

    def save_state(self, version: int): ...

    def save_column_sizes(self) -> None: ...
