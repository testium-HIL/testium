...
class ReportExportTxt(Exporter):
    ...
    def export(self):
        self._file_descriptor = open(self.out_path, 'w', encoding="utf-8")
    ...
    def write_row(self, row):
        if row.level > 0:
            msg = '' if row.passed else row.message
            if not row.skipped:
                self.line_result(row.name, row.result, msg, row.level)

            if row.type not in self.no_value_types:
                j = row.data
                if isinstance(j, dict):
                    for k, v in j.items():
                        self.line_value(row.name, k, '=', v, row.level)
    ...
    def write_header(self):
        ...
        self.addtxt('{:^96}'.format(
            self.report.header[cst.DB_TESTRUN_DATE] + ' ' +
            self.report.header[cst.DB_TESTRUN_TIME]) + '\n\n\n')
    ...
    def write_footer(self):
        ...
        self.addtxt('\n')
        succ = 'Not finished'
        if cst.DB_TEST_SET_RESULT in self.report.header:
            succ = self.report.header[cst.DB_TEST_SET_RESULT]
